...
class BERTModule(TFPluginAPI):
    ...
    def onJsonInput(self, jsonInput):
        print(jsonInput, self.tokenizer)
        input_string = jsonInput['text']
        label_ids, segment_ids, input_ids, input_mask = bertPreprocess(input_string, self.tokenizer)
        res = self.f(label_ids=label_ids, segment_ids=segment_ids, input_ids=input_ids, input_mask=input_mask)
        print(res['labels'].numpy())
        return {'prediction':int(res['labels'].numpy())}
