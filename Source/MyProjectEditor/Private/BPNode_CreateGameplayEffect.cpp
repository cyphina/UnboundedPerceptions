...
void UBPNode_CreateGameplayEffect::AllocateDefaultPins()
{
	...
	periodPin->DefaultValue = "0"; //Period is usually not a relevant value in timed effects but only periodic effects have executions, but modifiers will not be reset for periodic effects
	...
	UEdGraphPin* assetTagsPin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Struct, FGameplayTagContainer::StaticStruct(), *FBPNode_CreateGameplayEffectHelper::assetPinName);
	SetPinToolTip(*assetTagsPin, LOCTEXT("NamePinDesc", "Any additional tags this effect may have"));

	UEdGraphPin* resultPin = CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Struct, FGameplayEffectSpecHandle::StaticStruct(), *FBPNode_CreateGameplayEffectHelper::outputPinName);
...
bool UBPNode_CreateGameplayEffect::IsSpawnVarPin(UEdGraphPin* pin) const
{
	return(Super::IsSpawnVarPin(pin) &&
		pin->PinName != *FBPNode_CreateGameplayEffectHelper::periodPinName
		&& pin->PinName != *FBPNode_CreateGameplayEffectHelper::durationPinName
		&& pin->PinName != *FBPNode_CreateGameplayEffectHelper::abilityPinName
		&& pin->PinName != *FBPNode_CreateGameplayEffectHelper::levelPinName
		&& pin->PinName != *FBPNode_CreateGameplayEffectHelper::elemPinName
...
void UBPNode_CreateGameplayEffect::GetPinHoverText(const UEdGraphPin& Pin, FString& HoverTextOut) const
{
	...
	{
		SetPinToolTip(*PeriodPin, LOCTEXT("PeriodPinDescription", "Over the duration of the effect, will trigger the effect every time the period elapses.  0 means non periodic or instant effect, so set to higher than duration if this effect is not set to instant for duration policy but not periodic as well"));
	}
