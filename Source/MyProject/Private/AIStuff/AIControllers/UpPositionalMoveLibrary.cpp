...
bool UUpPositionalMoveLibrary::IsTargetInRange(const AActor& referenceActor, const FVector& targetLocation, const float range)
{
   FVector currentLocation = referenceActor.GetActorLocation();
   FVector difference      = currentLocation - targetLocation;
   difference.Z            = 0;

   if(FVector::DotProduct(difference, difference) <= range * range + SMALL_NUMBER)
      return true;
   return false;
}

bool UUpPositionalMoveLibrary::IsFacingTarget(const AActor& referenceActor, const FVector& targetLocation, const float angleErrorCutoff)
{
