...
=======
   bSuccess &= message == UnitMessages::AIMessage_SpellFinished & message != UnitMessages::AIMessage_SpellInterrupt & message != UnitMessages::AIMessage_TargetLoss;
>>>>>>> componentrefactor
...
=======
FString UBTTask_CastSpellByTags::GetStaticDescription() const
{
   FString keynames = "";
   for(const FGameplayTag& filterTag : filterTags)
   {
      keynames += filterTag.GetTagName().ToString() + "\n";
   }
   return FString::Printf(TEXT("%s: Filter Keys:\n%s"), *Super::GetStaticDescription(), *keynames);
}

TSubclassOf<UMySpell> UBTTask_CastSpellByTags::GetFirstCastableSpellMatchingTagCriteria(const FGameplayTagContainer& spellRequirementTags, const AUnit& ownerRef,
                                                                                        const USpellCastComponent*        spellCastComp,
...
=======
   for(int i = matchingSpellsStartIndex; i < equippedSpells.Num(); ++i)
   {
      if(const TSubclassOf<UMySpell> spellClass = abilitySystemComp->GetSpellAtSlot(i))
      {
         if(spellCastComp->CanCast(spellClass))
         {
>>>>>>> componentrefactor
            return spellClass;
         }
