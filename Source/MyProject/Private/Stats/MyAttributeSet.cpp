...
TArray<FGameplayAttributeData*> UMyAttributeSet::GetAtts() 
{
	TArray<FGameplayAttributeData*> atts = { &Strength, &Understanding, &Intelligence, &Explosiveness, &Endurance, &Agility, &Luck};
	return atts;
}

TArray<FGameplayAttributeData*> UMyAttributeSet::GetSkills() 
{
	TArray<FGameplayAttributeData*> skills = {&Critical_Chance,&Critical_Damage,&Accuracy,&Dodge,&Attack_Speed,&Cast_Speed,
	&Physical_Aff,&Fire_Aff,&Water_Aff,&Wind_Aff,&Earth_Aff,&Electric_Aff,&Darkness_Aff,&Light_Aff,&Arcane_Aff,&Chaos_Aff,&Poison_Aff,&Blood_Aff,&Ethereal_Aff,
	&Physical_Resist,&Fire_Resist,&Water_Resist,&Wind_Resist,&Earth_Resist,&Electric_Resist,&Darkness_Resist,&Light_Resist,&Arcane_Resist,&Chaos_Resist,&Poison_Resist,&Blood_Resist,&Ethereal_Resist };
	return skills;
}

TArray<FGameplayAttributeData*> UMyAttributeSet::GetVitals() 
{
	TArray<FGameplayAttributeData*> vits = {&Health,&Mana,&Psyche,&Moxie,&Shield};
	return vits;
}

TArray<FGameplayAttributeData*> UMyAttributeSet::GetMechanics() 
{
	TArray<FGameplayAttributeData*> mechs = {&MovementSpeed,&AttackRange,&WeaponPower,&GlobalDamageModifier };
	return mechs;
}
