...
void UEffectStatusBar::UpdateStatusBar()
{
   ...
   if (IsValid(CPCRef->GetBasePlayer()->focusedUnit)) {
      ...
      for (int i = 0; i < numDisplaySlots; ++i) {
         ...
         //Container which is passed by reference to hold asset tags (which includes name and element tags)
         ...
         for (FGameplayTag elemTag : elemTags) { finalColor = UpResourceManager::elementalMap[elemTag].WithAlpha(0.5); }
...
bool UEffectStatusBar::ActiveEffectHandleEqualityCheck(const FActiveGameplayEffectHandle& handle1, const FActiveGameplayEffectHandle& handle2) const { return handle1 == handle2; }
