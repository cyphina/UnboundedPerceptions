...
bool UConditionalManager::GetQuestConditionVal(FConditionData& condData) const
{
   ...
   if(gameModeRef->GetQuestManager()->GetCompletedQuests().FindByPredicate(
...
bool UConditionalManager::GetGoalsCompletedVal(FConditionData& condData) const
{
   ...
   if(gameModeRef->GetQuestManager()->GetCompletedQuests().FindByPredicate(pred)) return true;

   // If we can find this quest in our list of all quests, then
   if(auto questToCheck = gameModeRef->GetQuestManager()->GetActiveQuests().FindByPredicate(pred); questToCheck) {
      ...
      for(int i = 1; i < condData.conditionalValues.Num(); ++i) {
         ...
         if(quest->GetQuestInfo().GetSubgoals()[condIndex]->GetGoalState() != EGoalState::completedGoal) {
...
bool UConditionalManager::GetOwnedItemConditionVal(FConditionData& condData) const
{
   ...
   for(const ABaseHero* hero : cpcRef->GetBasePlayer()->GetHeroes()) {
      if(hero && hero->GetBackpack().FindItem(FCString::Atoi(*condData.conditionalValues[0])) != -1) return true;
   }
   return false;
}
