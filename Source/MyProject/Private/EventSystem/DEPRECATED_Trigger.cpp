...
{
   if(gameModeRef->HasAuthority() || !IsRunningDedicatedServer())
   {
...
void UTriggerManager::ChangeDialog(const FTriggerData& tdata)
{
   ...
   if(ANPC* finishedTalkNPC = UpResourceManager::FindTriggerObjectInWorld<ANPC>(tdata.triggerObjects[0], gameModeRef->GetWorld()))
   {
...
void UTriggerManager::ModifyStats(const FTriggerData& tdata)
{
   check(tdata.triggerObjects.Num() == 1 && tdata.triggerValues.Num() <= 2);
   uint8 offset = 0;
   if(AUnit* unit = UpResourceManager::FindTriggerObjectInWorld<AUnit>(tdata.triggerObjects[0], gameModeRef->GetWorld()))
...
void UTriggerManager::OpenHUDTrigger(const FTriggerData& tdata)
{
   ...
   hudManagerRef->AddHUD(static_cast<EHUDs>(TriggerValueToNum(tdata, 0)));
}
...
void UTriggerManager::ChangeParty(const FTriggerData& tdata)
{
   ...
   for(FString heroName : tdata.triggerObjects)
   {
      ...
      ABaseHero* hero = UpResourceManager::FindTriggerObjectInWorld<ABaseHero>(heroName, gameModeRef->GetWorld());
      if(hero)
      {
         newHeroes.Add(hero);
      }
...
bool UTriggerManager::CompleteQuestGoal(const FTriggerData& tdata)
{
   ...
   AQuest* quest = *gameModeRef->GetQuestManager()->GetActiveQuests().FindByPredicate(
       [&](AQuest* questToFilter)
       {
          return questToFilter->GetQuestInfo().id.ToString() == "QuestName." + tdata.triggerValues[0];
       });
...
void UTriggerManager::DestroyNPC(const FTriggerData& tdata)
{
   ...
   cpcRef->GetBasePlayer()->SetHeroBlockingInteraction(nullptr);
}
...
void UTriggerManager::AddItem(const FTriggerData& tdata)
{
   if(ensure(tdata.triggerValues.Num() == 2))
   {
      ...
      else
      {
         ...
         if(ABaseHero* activeHero = cpcRef->GetBasePlayer()->GetHeroBlockingInteraction(); activeHero) activeHero->backpack->AddItem(itemToAdd);
      }
   }
}
...
void UTriggerManager::SetNPCFollow(const FTriggerData& tdata)
{
   ...
   if(npcRef)
   {
      ...
      if(tdata.triggerObjects.Num() > 1)
      {
         ...
         heroRef = cpcRef->GetBasePlayer()->GetHeroBlockingInteraction();
...
void UTriggerManager::SetNPCWantConverse(const FTriggerData& tdata)
{
   ...
   if(npcRef)
   {
      npcRef->bWantsToConverse = static_cast<bool>(FCString::Atoi(*tdata.triggerValues[0]));
   }
}
...
void UTriggerManager::TriggerEffect(const FTriggerData& triggerData)
{
#if UE_EDITOR
