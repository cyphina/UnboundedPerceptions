...
void ABaseHero::BeginInteract(AActor* interactor)
{
   if (interactor->GetClass()->ImplementsInterface(UInteractable::StaticClass())) {
      ...
      UStaticMeshComponent* meshComp = currentInteractable->FindComponentByClass<UStaticMeshComponent>();
      ...
      if (meshComp && meshComp->Mobility != EComponentMobility::Movable)
         controller->MoveToLocation(IInteractable::Execute_GetInteractableLocation(currentInteractable, this));
      else
         controller->MoveToActor(currentInteractable, interactRange);
   }
}
...
void ABaseHero::PrepareInteract()
{
   ...
   if (!IsStunned() && currentInteractable) {
      FVector targetLoc = IInteractable::Execute_GetInteractableLocation(currentInteractable, this);
...
void ABaseHero::OnMoveCompleted(FAIRequestID RequestID, const EPathFollowingResult::Type Result)
{
   switch (GetState()) {
      case EUnitState::STATE_MOVING: state->ChangeState(EUnitState::STATE_IDLE); break;
      case EUnitState::STATE_INTERACTING:
         if (currentInteractable) {
            if (Result == EPathFollowingResult::Success) {
               TurnTowardsTarget(currentInteractable->GetActorLocation());
...
bool ABaseHero::CastSpell(TSubclassOf<UMySpell> spellToCast)
{
   ...
   if (itemID && sucessfulCast) UseItem(itemID);
