...
struct FBackpackUpdateResult
{
   ...
   FBackpackUpdateResult() :
      updatedBackpackIndices(TArray<int>()), bSuccessfulOperation(false), itemId(-1), numUpdatedItemsRequested(0), numUpdatedItemsRemaining(0)
   {
   }

   explicit FBackpackUpdateResult(const int newItemCount) :
      FBackpackUpdateResult()
   {
      this->numUpdatedItemsRemaining = newItemCount;
   }

   explicit FBackpackUpdateResult(const FMyItem& itemBeforeUpdate) :
      FBackpackUpdateResult()
   {
   ...
   FBackpackUpdateResult(const TArray<int>& updated_backpack_indices, bool b_successful_operation, int item_id, int num_updated_items_requested, int num_updated_items_remaining) :
      updatedBackpackIndices(updated_backpack_indices),
      bSuccessfulOperation(b_successful_operation),
      itemId(item_id),
      numUpdatedItemsRequested(num_updated_items_requested),
