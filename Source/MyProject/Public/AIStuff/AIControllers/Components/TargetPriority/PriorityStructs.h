...
struct FSpellTargetCriteria {
   ...
   FORCEINLINE TArray<FTargetTest*>& GetCriteriaFromSpellDescTag(FGameplayTag descTag);

 private:
  const inline TMap<const FGameplayTag&, const TArray<FTargetTest*>&> descTagToTargetTestsMap = {
     {FGameplayTag::RequestGameplayTag("Skill.Category.Offensive"), offensiveTests}
  };

};

inline TArray<FTargetTest*>& FSpellTargetCriteria::GetCriteriaFromSpellDescTag(FGameplayTag descTag)
{
